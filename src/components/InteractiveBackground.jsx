...
export default function InteractiveBackground() {
  ...
  useEffect(() => {
    ...
    const initNodes = () => {
      ...
      const count = Math.min(130, Math.max(55, Math.round(area / 12000)))
      ...
      triangles = []
      for (let i = 0; i < Math.round(count * 0.16); i++) {
        const a = nodes[Math.floor(Math.random() * nodes.length)]
        const b = nodes[Math.floor(Math.random() * nodes.length)]
        const cc = nodes[Math.floor(Math.random() * nodes.length)]
        if (a !== b && b !== cc && a !== cc) {
          const d1 = Math.hypot(a.x - b.x, a.y - b.y)
          const d2 = Math.hypot(b.x - cc.x, b.y - cc.y)
          const d3 = Math.hypot(a.x - cc.x, a.y - cc.y)
          if (d1 < 250 && d2 < 250 && d3 < 250) {
            triangles.push({ a, b, c: cc, alpha: 0.02 + Math.random() * 0.04 })
          }
        }
      }
    }
    ...
    const animate = () => {
      ...
      const t = Math.min(scrollRef.current / 1200, 1)
      const zoom = 1 + t * 0.35
      const fade = 1 - t * 0.25
      ...
      nodes.forEach(n => {
        ...
        const m = 40
        if (n.x < -m) n.baseX = n.x = width + m
        if (n.x > width + m) n.baseX = n.x = -m
        if (n.y < -m) n.baseY = n.y = height + m
        if (n.y > height + m) n.baseY = n.y = -m
      })
      ...
      for (let i = 0; i < nodes.length; i++) {
        for (let j = i + 1; j < nodes.length; j++) {
          ...
          if (dist < LINK_DIST) {
            ...
            ctx.moveTo(a.x, a.y)
            ctx.lineTo(b.x, b.y)
            ctx.strokeStyle = `rgba(${baseRGB},${(1 - dist / LINK_DIST) * 0.18 * fade})`
            ctx.lineWidth = 0.5
            ctx.stroke()
          }
        }
        if (mx > 0) {
          ...
          if (dist < 170) {
            ...
            ctx.lineTo(mx, my)
            ctx.strokeStyle = `rgba(${accentRGB},${(1 - dist / 170) * 0.4 * fade})`
            ctx.lineWidth = 0.7
            ctx.stroke()
          }
        }
      ...
      nodes.forEach(n => {
        ...
        if (mx > 0) {
          ...
          boost = Math.exp(-d2 / 12000)
        }
        const radius = n.r * (1 + boost * 0.8)
        const alpha = (0.4 * flick + boost * 0.5) * fade
        const glowR = radius * (2.2 + boost * 2)
